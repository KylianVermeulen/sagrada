...
public class PlayerDao {
  ...
  public ArrayList<Player> getPlayersOfAccount(Account account) {
    ...
    try {
      ResultSet rs = dbConnection.executeQuery(new Query("SELECT * FROM player WHERE username=?",
          "query", new QueryParameter(QueryParameter.STRING, account.getUsername())));
      while (rs.next()) {
  ...
  public void updatePlayer(Player player) {
    try {
      ResultSet rs = dbConnection.executeQuery(new Query(
          ...
          "update"), new QueryParameter(QueryParameter.STRING, player.getAccount().getUsername()),
  ...
  public Player getPlayerById(int id) {
    Player player = new Player();
    try {
      ResultSet rs = dbConnection.executeQuery(new Query("SELECT * FROM player WHERE idplayer=?",
  ...
  public int getNextPlayerId() {
    try {
      ResultSet rs = dbConnection
          .executeQuery(new Query("SELECT MAX(idplayer) AS highestId FROM player", "query"));
      if (rs.next()) {
  ...
  public Player getPlayerByAccountAndGame(Account account, Game game) {
    ...
    try {
      ResultSet rs = dbConnection.executeQuery(
          new Query("SELECT idplayer FROM player WHERE username=? AND game_idgame=?", "query"),

...
public class PlayerController {
	...
	public void viewOptionalPatternCards() {
		Pane pane = new Pane();
		ArrayList<PatternCard> patternCards = new PatternCardDao().getOptionalPatternCardsOfPlayer(player);
	...
	public void sendMessage(String text) {
		...
		if (!text.matches("")) {
			if (chatlineDao.timeExistsOfPlayer(chatline) == false) {
				...
				Alert alert = new Alert("Waarschuwing", "Je mag maar 1 keer per seconde een bericht versturen!",
						AlertType.ERROR);
